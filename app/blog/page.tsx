...
export const metadata: Metadata = {
  title: "Medicare & Medicaid Guides & Resources",
  description:
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-slate-900 mb-2">
        Medicare &amp; Medicaid Guides &amp; Resources
      </h1>
      <p className="text-slate-600 mb-8">
        Expert articles on Medicare plans, costs, enrollment deadlines, and
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {categories.map((cat) => (
          <span
            key={cat}
            className="text-xs px-3 py-1 bg-teal-50 text-teal-700 border border-teal-200 rounded-full"
          >
            {cat}
          </span>
        ))}
        ...
        {posts.map((post) => (
          ...
          >
            <article className="border border-slate-200 rounded-xl p-6 hover:border-teal-300 hover:shadow-sm transition-all">
              ...
              <h2 className="text-lg font-semibold text-slate-900 group-hover:text-teal-700 mb-2 leading-snug">
                {post.title}
              </h2>
