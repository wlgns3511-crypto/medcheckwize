  ...
  return (
    <article className="prose prose-slate max-w-3xl mx-auto">
      ...
      <nav className="not-prose text-sm text-slate-500 mb-4">
        ...
        <Link href="/guide/" className="hover:text-teal-700">Guides</Link>
        <span className="mx-2">›</span>
      ...
      <TrustBlock sources={[{name:"CMS Medicare PFS",url:"https://www.cms.gov/medicare/payment/fee-schedules/physician"},{name:"Medicare.gov",url:"https://www.medicare.gov/care-compare/"},{name:"AMA CPT",url:"https://www.ama-assn.org/practice-management/cpt"}]} updated="CMS PFS 2026" />
      ...
      <nav aria-label="Table of contents" className="not-prose my-6 p-4 rounded-xl bg-slate-50 border border-slate-200">
        ...
        <ol className="space-y-1.5 text-sm">
          {guide.sections.map((s, i) => (
            <li key={i}>
              <a href={`#section-${i}`} className="text-teal-700 hover:underline">{s.heading}</a>
            </li>
          ))}
          <li><a href="#faq" className="text-teal-700 hover:underline">Frequently Asked Questions</a></li>
        </ol>
      </nav>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {others.map((g) => (
              <Link
                key={g.slug}
                href={`/guide/${g.slug}/`}
                className="block rounded-lg border border-slate-200 hover:border-teal-400 hover:bg-teal-50 p-4 transition-colors"
              >
